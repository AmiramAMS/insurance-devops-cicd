...
interface Props {
  ...
  onSave: (
    id: string,
    patch: { status: ProcessStatus; requiredDate: string; assignedAgent: string }
  ) => void;
...
export function ProcessDetailsDialog({
  ...
  useEffect(() => {
    if (process) {
      ...
      setAssignedAgent(process.assignedAgent ?? AGENTS[0]);
      setNoteText("");
    }
  }, [process]);
  ...
  const handleSave = () => {
    onSave(process.id, { status, requiredDate, assignedAgent });
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2 border-t border-border">
          ...
          <div className="space-y-2">
            <Label htmlFor="assigned-agent-select">סוכן מטפל</Label>
            <Select value={assignedAgent} onValueChange={setAssignedAgent}>
              <SelectTrigger id="assigned-agent-select" data-testid="assigned-agent-select">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {AGENTS.map((a) => (
                  <SelectItem key={a} value={a} data-testid={`agent-option-${a}`}>
                    {a}
                  </SelectItem>
                ))}
              </SelectContent>
